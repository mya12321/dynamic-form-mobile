<template>
  <div class="content" :style="{height: clientHeight+'px'}">
    <div class="item">
      <div class="title">旧印章密码</div>
      <div class="input"><input type="password" v-model="oldPwd" oninput="value=value.replace(/[^\d]/g,'')" maxlength="6" placeholder="创建企业时的密码，6位数字"></div>
    </div>
    <div class="item">
...
<script>
  export default {
    ...
    methods: {
      submit(){
        if (this.oldPwd == "" || this.oldPwd.length<6){
          this.$alert("请输入正确的6位数旧密码");
        }else if (this.newPwd == "" || this.newPwd.length<6){
          this.$alert("请输入正确的6位数新密码");
        } else{
          var remit = this.$post('api/user/company/change/pwd', {
